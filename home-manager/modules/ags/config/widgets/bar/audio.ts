...
import { audio } from "utils/services";
...
const AudioTray = () => {
  const AudioButtons = playback.bind("sinks").as((sinks) => {
    const sinkList = sinks.map((sink) => {
      return Widget.Button({
        hpack: "start",
        vpack: "start",
        label: `${sink.sinkNick} ${sink.sinkId}`,
        className: `${sink.sinkActive ? "activeSink" : ""}`,
        onClicked: async () => {
          print(`wpctl set-default ${sink.sinkId}`);
          Utils.execAsync(`wpctl set-default ${sink.sinkId}`);
  ...
  const MainBox = Widget.Box({
    ...
    hpack: "center",
    css: "padding: 15px;",
    children: [Slider, AudioButtonBox],
  });
